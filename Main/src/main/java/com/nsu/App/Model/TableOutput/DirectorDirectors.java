...
public class DirectorDirectors implements Table {
    final String request = "select FIRST_NAME, SECOND_NAME, MIDDLE_NAME, START_CONTRACT, END_CONTRACT, EXPERIANCE, SEX, DATE_OF_BIRTH, NUMBER_OF_CHILDREN, SALARY\n" +
                            "from employees e\n" +
                            "join DIRECTOR_DIRECTORS dd on dd.ID_DIRECTOR_DIRECTOR = e.ID_EMPLOYEE";
    ...
    public void displayTable(TableView rightTable) throws SQLException {
        ...
        for (int i = 0; i < tableData.size() - rowCount + 1; i += rowCount){
            ...
            strings.add(new TableColumns(tableData.get(i), tableData.get(i + 1), tableData.get(i + 2), tableData.get(i + 3), tableData.get(i + 4),
                    numberOne, tableData.get(i + 6), tableData.get(i + 7), numberTwo, numberThree));
        }
