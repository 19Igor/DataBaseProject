...
public class FilmDirectors implements Table {

    final String request = "select FIRST_NAME, SECOND_NAME, MIDDLE_NAME, AMOUNT_OF_PERF, START_CONTRACT, END_CONTRACT, EXPERIANCE, SEX, DATE_OF_BIRTH, NUMBER_OF_CHILDREN, SALARY\n" +
                            "from employees e\n" +
                            "join FILM_DIRECTORS fd on e.ID_EMPLOYEE = fd.ID_FILM_DIRECTOR";
    ...
    public void displayTable(TableView rightTable) throws SQLException {
        ...
        for (int i = 0; i < tableData.size() - rowCount + 1; i += rowCount){
            ...
            if (tableData.get(i + 3) != "--")
                numberOne = Integer.parseInt(tableData.get(i + 3));

            if (tableData.get(i + 6) != "--")
                numberTwo = Integer.parseInt(tableData.get(i + 6));

            if (tableData.get(i + 9) != "--")
                numberThree = Integer.parseInt(tableData.get(i + 9));

            if (tableData.get(i + 10) != "--")
                numberFour = Integer.parseInt(tableData.get(i + 10));


            strings.add(new TableColumns(tableData.get(i), tableData.get(i + 1), tableData.get(i + 2),
                    numberOne, tableData.get(i + 4), tableData.get(i + 5),
                    numberTwo, tableData.get(i + 7), tableData.get(i + 8),
        ...
        TableColumn<TableColumns, Integer> playNumb = new TableColumn<>("Amount of realised plays");
        playNumb.setCellValueFactory(new PropertyValueFactory<>("playAmount"));
        rightTable.getColumns().add(playNumb);
    ...
    public class TableColumns{
        ...
        private final SimpleIntegerProperty playAmount;
        ...
        public TableColumns(String firstName, String secondName, String middleName, int playAmount, String startContract,
                            String endContract, int experiance, String sex, String birthDate, int numberOfChildren, int salary) {
            this.firstName          = new SimpleStringProperty(firstName);
            this.secondName         = new SimpleStringProperty(secondName);
            this.middleName         = new SimpleStringProperty(middleName);
            this.playAmount         = new SimpleIntegerProperty(playAmount);
