...
public class Employees implements Table {
    ...
    final String request = "select ID_EMPLOYEE, FIRST_NAME, SECOND_NAME, MIDDLE_NAME, START_CONTRACT, END_CONTRACT, " +
                            "NAME, EXPERIANCE, SEX, DATE_OF_BIRTH, NUMBER_OF_CHILDREN, SALARY\n" +
                            "from employees em\n" +
                            "join position po on (em.ID_EMPLOYEE = po.ID_POSITION)";
    ...
    public void displayTable(TableView rightTable) throws SQLException {
        ...
        for (int i = 0; i < tableData.size() - rowCount + 1; i += rowCount){
            ...
            if (tableData.get(i + 7) != "--")
                numberOne = Integer.parseInt(tableData.get(i + 7));

            if (tableData.get(i + 10) != "--")
                numberTwo = Integer.parseInt(tableData.get(i + 10));

            if (tableData.get(i + 11) != "--")
                numberThree = Integer.parseInt(tableData.get(i + 11));

            strings.add(new TableColumns(Integer.parseInt(tableData.get(i)), tableData.get(i + 1),
                    tableData.get(i + 2), tableData.get(i + 3), tableData.get(i + 4), tableData.get(i + 5),
                    tableData.get(i + 6), numberOne,tableData.get(i + 8), tableData.get(i + 9), numberTwo, numberThree));

        }

        TableColumn<TableColumns, Integer> idE = new TableColumn<>("ID");
        idE.setCellValueFactory(new PropertyValueFactory<>("idEmpl"));
        rightTable.getColumns().add(idE);
        ...
        TableColumn<TableColumns, String> pos = new TableColumn<>("Position");
        pos.setCellValueFactory(new PropertyValueFactory<>("position"));
        rightTable.getColumns().add(pos);
    ...
    public class TableColumns{
        private final SimpleIntegerProperty idEmpl;
        ...
        public TableColumns(int id, String firstName, String secondName, String middleName, String startContract,
                            String endContract, String position, int experiance, String sex, String birthDate,
                            int numberOfChildren, int salary) {
            this.idEmpl             = new SimpleIntegerProperty(id);
        ...
        public SimpleIntegerProperty idEmplProperty() {
            return idEmpl;
        }
